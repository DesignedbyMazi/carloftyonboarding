  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      <div className="flex flex-col gap-0.5">
        <label className="text-sm font-medium text-[#2d2d2d] tracking-[0.14px]">{label}</label>
        {description && <p className="text-xs text-[#959595] tracking-[0.24px]">{description}</p>}
      </div>
      <div className="flex flex-col gap-2">
        {options.map((opt) => (
          <label
            key={opt.value}
            className="flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer hover:border-[#d1d5db] transition-colors"
            style={{ borderColor: value === opt.value ? "#2d2d2d" : "#eaeaea" }}
          >
            <div
              className="shrink-0 w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors"
              style={{ borderColor: value === opt.value ? "#2d2d2d" : "#d1d5db", backgroundColor: value === opt.value ? "#2d2d2d" : "white" }}
            >
    ...
    <FinalStepLayout activeStep="account-usage">
      <div className="flex flex-col gap-8 lg:gap-10 items-center w-full max-w-[472px]">
        <div className="flex flex-col gap-2 items-center text-center w-full">
          <h1 className="text-xl lg:text-2xl font-medium text-[#2d2d2d] leading-8">Expected Account Usage</h1>
          <p className="text-sm text-[#5a5a5a] leading-5 tracking-[0.14px]">Tell us how you plan to use your account.</p>
        </div>

        <div className="flex flex-col gap-5 lg:gap-6 w-full">
          <SelectField
            label="Purpose of Account" placeholder="Select purpose"
            value={accountPurpose} onChange={setAccountPurpose}
            options={["Personal Payments","Business Payments","International Transfers","Savings","Investment","Other"]}
          />
          <SelectField
            label="Expected Monthly Payment Volume" placeholder="Select monthly volume"
            value={monthlyVolume} onChange={setMonthlyVolume}
            options={["Less than $1,000","$1,000 – $5,000","$5,000 – $10,000","$10,000 – $50,000","More than $50,000"]}
          />
          <RadioGroup
            ...
            value={intermediary} onChange={setIntermediary}
          />
          ...
          <label className="flex items-start gap-3 cursor-pointer select-none">
            <div
              ...
              style={{ borderColor: confirmed ? "#2d2d2d" : "#d1d5db", backgroundColor: confirmed ? "#2d2d2d" : "white" }}
            >
              {confirmed && (

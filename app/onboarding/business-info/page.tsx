...
type IdentityMethod = "" | "tin" | "cac" | "utility";
        ...
        <div className="flex flex-col items-center justify-center gap-3 py-8 px-6 text-center">
          {file ? (
            <>
              ...
              <button onClick={() => inputRef.current?.click()} className="text-xs text-[#6b7280] underline hover:text-[#374151]">
                Replace file
              </button>
            </>
          ) : (
            <>
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-[#374151] bg-white border border-[#d1d5db] rounded-lg hover:bg-[#f9fafb] shadow-sm"
              >
                Browse files
              </button>
            </>
...
export default function BusinessInfoPage() {
  ...
  const isValid =
    ...
    (method === "cac"     && cacReg.trim().length > 0 && cacFile !== null) ||
        ...
        <div className="flex flex-col gap-5 w-full">
          ...
          <div className="flex flex-col gap-1.5 w-full">
            ...
            <div className="relative">
              <select
                value={method}
                onChange={(e) => { setMethod(e.target.value as IdentityMethod); }}
          ...
          {method === "cac" && (
            <>
              <div className="flex flex-col gap-1.5 w-full">
                <div className="flex items-center gap-px text-sm leading-5">
                  <span className="font-medium text-[#2d2d2d] tracking-[0.21px]">CAC Registration Number</span>
                  <span className="font-medium text-[#335cff]">*</span>
                </div>
                <input
                  type="text"
              ...
              <UploadZone file={cacFile} onFile={setCacFile} label="Upload CAC Document" />
            </>
          )}

          {/* Utility Bill method */}
          {method === "utility" && (
            <UploadZone file={utilityFile} onFile={setUtilityFile} label="Upload Utility Bill" />
          )}
        </div>
